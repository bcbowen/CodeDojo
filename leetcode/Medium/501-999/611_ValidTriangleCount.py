...
class Solution:
    @staticmethod
    # Find first index greater than the value
    def find_first(nums: List[int], val: int) -> int:
        left = 0
        right = len(nums) - 1
        ans = -1  # default if no element greater than val is found
    ...
    def triangleNumber(self, nums: List[int]) -> int:
        ...
        for i in range(len(nums) - 1): 
            for j in range(i + 1, len(nums)):
                val = nums[i] + nums[j]
                k = Solution.find_first(nums, val)
                tri_count += k - j
...
def test_triangleNumber(nums: List[int], expected: int):
...
def test_find_first(nums: List[int], val: int, expected: int): 
    result = Solution.find_first(nums, val)
    assert(result == expected)
