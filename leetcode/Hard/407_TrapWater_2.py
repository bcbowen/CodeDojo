...
class Solution:
    ...
    def trapRainWater(self, heightMap: list[list[int]]) -> int:
        ...
        for i in range(col_count):
            ...
            heapq.heappush(boundary, self.Cell(heightMap[row_count - 1][i], i, row_count - 1))
            visited[0][i] = visited[0][row_count - 1] = True
    ...
    def trapRainWater2(self, height_map):
        # Direction arrays
        d_row = [0, 0, -1, 1]
        d_col = [-1, 1, 0, 0]
...
def test_trapRainWater2(heightMap: list[list[int]], expected: int): 
    solution = Solution()
    result = solution.trapRainWater2(heightMap)
    assert(result == expected)
