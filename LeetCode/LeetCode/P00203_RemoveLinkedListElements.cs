using LeetCode.Solutions.Models.LinkedList;
using LeetCode.Solutions.P00203_RemoveLinkedListElements;
using LeetCode.Tests.Utility.Helpers;
namespace LeetCode.Tests.P00203_RemoveLinkedListElements;

public class Tests
{
    [TestCase(new[] { 1, 2, 6, 3, 4, 5, 6 }, 6, new[] { 1, 2, 3, 4, 5 })]
    [TestCase(new[] { 1, 2, 6, 3, 4, 5, 6 }, 1, new[] { 2, 6, 3, 4, 5, 6 })]
    [TestCase(new int[0], 1, new int[0])]
    [TestCase(new[] { 7, 7, 7, 7 }, 7, new int[0])]
    public void ReverseListTest(int[] values, int value, int[] expected)
    {
        ListNode start = LinkedListHelpers.Init(values);
        ListNode result = new Solution().RemoveElements(start, value);

        if (values.Length > 0)
        {
            ListNode current = result;
            int i = 0;
            while (current != null)
            {
                Assert.AreEqual(expected[i++], current.val);
                current = current.next;
            }
        }
        else
        {
            Assert.Null(result);
        }

    }


}