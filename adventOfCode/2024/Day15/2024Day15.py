...
def load_input(file_name: str) -> tuple[list[list[str]], list[str]]:
    content = Modules.aoc_io.read_input(2024, 15, file_name)
...
def move_robot(
    grid: list[list[str]], current_location: Point, direction: Direction
) -> tuple[Point, list[list[str]]]:
    next_location = Point(
        y=current_location.y + direction.y, x=current_location.x + direction.x
    )
    ...
    while grid[test_location.y][test_location.x] == "O":
        ...
        test_location = Point(
            test_location.y + direction.y, test_location.x + direction.x
        )
...
@pytest.mark.parametrize(
    "file_name, expected", [("sample4.txt", 10092), ("sample3.txt", 2028)]
)
...
@pytest.mark.parametrize(
    "row_in, start_position, end_position, row_out",
    [
        ("..@..", Point(0, 2), Point(0, 3), "...@."),
...
def test_move_robot2(
    row_in: str, start_position: Point, end_position: Point, row_out: str
):
    grid_in = []
