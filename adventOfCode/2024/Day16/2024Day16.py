...
def load_inputs(file_name: str) -> list[list[str]]:
    return [
        list(line.strip())
        for line in Modules.aoc_io.read_input(2024, 16, file_name).split("\n")
    ]
...
def day16(file_name: str) -> tuple[int, int]:
    ...
    while len(position_q) > 0:
        ...
        if grid[current_position.location.y][current_position.location.x] == "E":
            ...
            next_location = Point(
                current_position.location.y + current_position.orientation.y,
                current_position.location.x + current_position.orientation.x,
            )
            edge = Edge(
                (current_position.location.y, current_position.location.x),
                (next_location.y, next_location.x),
            )
            reversed_edge = edge.get_reversed
            if (
                not reversed_edge in seen
                and grid[next_location.y][next_location.x] != "#"
            ):
                seen.add(edge)
                position = Position(
                    current_position.cost + 1,
                    next_location,
                    current_position.orientation,
                    current_position.path.copy(),
                )
                if current_position.cost > min_cost:
                    continue
            ...
            for orientation in turns:
                next_location = Point(
                    current_position.location.y + orientation.y,
                    current_position.location.x + orientation.x,
                )
                edge = Edge(
                    (current_position.location.y, current_position.location.x),
                    (next_location.y, next_location.x),
...
def main():
    ...
    print(
        f"{file_name} min path length (part 1): {min_path}; tiles used for min paths: {tiles_used} (part 2)"
    )


@pytest.mark.parametrize(
    "file_name, expected_cost, expected_tiles_used",
    [("sample1.txt", 7036, 45), ("sample2.txt", 11048, 64)],
)
