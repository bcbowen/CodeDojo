...
def load_input(file_name: str) -> list[Point]:
    content = Modules.aoc_io.read_input(2024, 18, file_name)
    points = [
...
def find_path(grid: list[list[str]]) -> int:
    def try_step(position: Point, direction: Direction, current_count: int):
        ...
        if (
            inbounds(next_position, grid_size)
            and not next_position in seen
            and grid[next_position.y][next_position.x] == "."
        ):
...
def main():
    ...
    difference = end_time - start_time
    print(
        f"Part 2 result for {file_name}(x, y): {result.x}, {result.y} in {difference.seconds} seconds"
    )
