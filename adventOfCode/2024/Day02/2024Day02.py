...
def is_safe(report: list[int], tolerance: int) -> bool:
    ...
    if diffs[0] == 0:
        if tolerance == 0:
            return False
        return is_safe(report[0:1] + report[2:], tolerance - 1) or is_safe(
            report[1:], tolerance - 1
        )
    ...
    for i in range(len(diffs)):
        if (
            (increasing and diffs[i] <= 0)
            or (not increasing and diffs[i] >= 0)
            or abs(diffs[i]) > 3
        ):
            if tolerance == 0:
                return False
            if i == 1:
                return (
                    is_safe(report[0:1] + report[2:], tolerance - 1)
                    or is_safe(report[0:2] + report[3:], tolerance - 1)
                    or is_safe(report[1:], tolerance - 1)
                )
            elif i == len(diffs) - 1:
                return is_safe(report[0:i] + report[i + 1 :], tolerance - 1) or is_safe(
                    report[0 : i + 1], tolerance - 1
                )
            else:
                return is_safe(report[0:i] + report[i + 1 :], tolerance - 1) or is_safe(
                    report[0 : i + 1] + report[i + 2 :], tolerance - 1
                )
    return True


def day2(file_name: str, tolerance: int) -> int:
    content = Modules.aoc_io.read_input(2024, 2, file_name)
    parsed_reports = [
...
def part2_troubleshooting():
    file_name = "input.txt"
    content = Modules.aoc_io.read_input(2024, 2, file_name)
    output_path = file_name.replace("input.txt", "output.txt")
...
@pytest.mark.parametrize("tolerance, expected", [(0, 2), (1, 4)])
