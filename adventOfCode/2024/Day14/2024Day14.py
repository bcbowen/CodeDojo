...
class Robot:
    ...
    def calc_future_position(
        self, turns: int, grid_height: int, grid_width: int
    ) -> Position:
        x = self.position.x
        y = self.position.y
...
def load_robots(file_name: str) -> list[Robot]:
    content = Modules.aoc_io.read_input(2024, 14, file_name)
    id = 1
    robots = []
...
def main():
    ...
    result = part2()
    print(
        f"Part 2 result for {file_name}: Iteration (1-based) where the robots are arranged in a tree: {result}"
    )
...
@pytest.mark.parametrize(
    "start_x, start_y, v_x, v_y, iterations, expected_x, expected_y",
    [
        (2, 4, 2, -3, 1, 4, 1),
        (2, 4, 2, -3, 2, 6, 5),
        (2, 4, 2, -3, 3, 8, 2),
...
@pytest.mark.parametrize("width, height", [(101, 103), (11, 7)])
