...
def load_grid(file_name: str) -> list[list[str]]:
    content = Modules.aoc_io.read_input(2024, 8, file_name)
...
def map_antinodes(
    station_map: dict[str, list[tuple[int, int]]], grid_size: int
    ...
    def calc_antinodes(
        a: tuple[int, int], b: tuple[int, int], grid_size
    ) -> set[tuple[int, int]]:
        dy = b[0] - a[0]
        dx = b[1] - a[1]
        nodes = set()
        ...
        if (
            node1[0] >= 0
            and node1[0] < grid_size
            and node1[1] >= 0
            and node1[1] < grid_size
        ):
            nodes.add(node1)
            all_nodes.add(node1)
        ...
        if (
            node2[0] >= 0
            and node2[0] < grid_size
            and node2[1] >= 0
            and node2[1] < grid_size
        ):
            nodes.add(node2)
            all_nodes.add(node2)
    ...
    for station_key in station_map:
        ...
        for i in range(len(station_map[station_key]) - 1):
            for j in range(i + 1, len(station_map[station_key])):
                nodes = calc_antinodes(
                    station_map[station_key][i], station_map[station_key][j], grid_size
                )
