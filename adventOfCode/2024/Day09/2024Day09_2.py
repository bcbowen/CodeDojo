...
def move_file(drive: list[str], source: tuple[int, int], destination: tuple[int, int]):
    for i in range(source[1] - source[0] + 1):
        drive[source[0] + i], drive[destination[0] + i] = (
            drive[destination[0] + i],
            drive[source[0] + i],
        )
...
@pytest.mark.parametrize(
    "input, space, expected, expected_location",
    [
...
def test_find_space(
    input: str, space: int, expected: bool, expected_location: tuple[int, int]
):
    drive = fill_drive(input)
