...
long Part1(string fileName)
{
	...
	List<SeedLocation> locationGrid = SeedLocation.BuildSeedLocationGrid(map);
	long result = locationGrid.Min(g => g.LocationId);
	Console.WriteLine($"Part1 Result: {result}");
	return locationGrid.Min(g => g.LocationId);
}
...
class SeedLocation
{
	...
	public static List<SeedLocation> BuildSeedLocationGrid(NodeMap map)
	{
		List<SeedLocation> result = new List<SeedLocation>();

		foreach (long seed in map.Seeds)
		{
			SeedLocation location = new SeedLocation { SeedId = seed };
			// soil
			NodeList currentNodeList = map.Map[0];
			location.SoilId = FindIdInMap(currentNodeList, seed);
...
class NodeMap
{
	...
	public static NodeMap Load(string fileName)
	{
		...
		*/
		...
		{
			map.Seeds.Add(long.Parse(match.Value));
		}
