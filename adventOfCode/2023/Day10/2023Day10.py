...
def load_map(file_name: str) -> list[list[str]]:
    input_map = []
    content = Modules.aoc_io.read_input(2023, 10, file_name)
    input_map = [list(line.strip()) for line in content.splitlines(keepends=True)]
...
def test_get_direction(
    start: tuple[int, int], end: tuple[int, int], expected: Direction
):
...
@pytest.mark.parametrize(
    "file_name, expected", [("sample.txt", (1, 1)), ("sample2.txt", (2, 0))]
)
