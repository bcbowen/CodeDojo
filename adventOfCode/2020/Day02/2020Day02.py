...
def load_inputs(file_name: str) -> list[PasswordValidation]:
    inputs = [
        PasswordValidation(line.strip())
        for line in Modules.aoc_io.read_input(2020, 2, file_name).split("\n")
    ]
    return inputs
...
@pytest.mark.parametrize(
    "line, min, max, char, password, should_be_valid",
    [
...
def test_init(
    line: str, min: int, max: int, char: str, password: str, should_be_valid: bool
):
