...
void Main()
{
	...
	int accumulator = FixProgram(program);
	
	Console.WriteLine($"accumulator: {accumulator}");
}

int FixProgram(List<Instruction> program) 
{
	Result result = TryProgram(program);

	while (result.IsFaulted)
	{
		for (int i = 0; i < program.Count(); i++)
		{
			if (program[i].Command == "nop" || program[i].Command == "jmp")
			{
				program[i].JumpToggle();
				result = TryProgram(program);
				if (result.IsFaulted) 
				{
					program[i].JumpToggle();
				}
...
Result TryProgram(List<Instruction> program) 
{ 
	int accumulator = 0;
	int pointer = 0;
	bool faulted = false;
